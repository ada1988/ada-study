...
public class RegionBaseTask<T> extends RecursiveTask<Integer> {
	 ...
	 *            为每个分区设定数据块的大小
	 */
	public void init(List<T> datas,String mainName,IRegionWork<T> regionWork,Integer regionLength){
		if(null!= datas && datas.size()>0&&null!=mainName){
			this.datas = datas;
	...
	private void selfInit(final List<T> datas, final String mainName, final String subName, final IRegionWork<T> regionWork,
			final Integer regionLength) {
		this.datas = datas;
		this.mainName = mainName;
	...
	public void dataRegion(List<T> datas, List<RegionBaseTask<T>> subTasks) {
		...
		RegionBaseTask<T> task = new RegionBaseTask<T>();
		if (null != subDatas && subDatas.size() > regionLength) {
			/**
			 * 为子任务拆分数据
			 */
			task.selfInit( subDatas.subList( 0, regionLength ), this.mainName, this.mainName + "-task-"
					+ taskNums.get( mainName ), this.regionWork, this.regionLength );
			subTasks.add( task );
			taskNums.put( mainName, taskNums.get( mainName ) + 1 );// 任务序号
			dataRegion( subDatas, subTasks );// 递归：数据分区，分割子任务
		} else {
			task.selfInit( subDatas.subList( 0, subDatas.size() ), this.mainName, this.mainName + "-task-"
					+ taskNums.get( mainName ), this.regionWork, this.regionLength );
			subTasks.add( task );
			taskNums.put( mainName, taskNums.get( mainName ) + 1 );// 任务序号
		}
